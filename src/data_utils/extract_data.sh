...
for part in 1 2; do
  ...
  for speaker_dir in "$common_base"/WAVE/*; do
    ...
    for session in 0 1; do
      ...
      wav_scp_file="$OUTPUT_DIR"/"$part"-"$speaker_session"-wav.scp
      ...
      if [ ! -f "$part"-"$speaker_session".data ]; then
        ...
        KALDI_ROOT=$PATH_TO_KALDI python3 format_session_data.py \
          "$speaker_session_transcript" energy.scp pitch.scp "$OUTPUT_DIR"/"$part"-"$speaker_session".data
      fi
